...
app.get('/uploads', async (_req, res, next) => {
  try {
    await ensureStorage();
    const objects = await listUploads();
    res.json({ items: objects.map((o) => ({ objectName: o.name, lastModified: o.lastModified, comment: o.comment || '' })) });
  } catch (err) {
    next(err);
  }
});
...
app.post('/media', upload.single('file'), async (req, res, next) => {
  try {
    ...
    const comment = req.body && req.body.comment ? String(req.body.comment).substring(0, 500) : '';
