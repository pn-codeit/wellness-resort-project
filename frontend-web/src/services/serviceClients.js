...
async function getImpressions(lang) {
  ...
  let uploadItems = [];
  try {
    const uploads = await requestJson(`${serviceUrls.impressions}/uploads`);
    if (uploads && uploads.items) {
      uploadItems = uploads.items.map((u) => ({
        cat_de: 'Gästefotos',
        cat_en: 'Guest Photos',
        label_de: 'Gästefoto',
        label_en: 'Guest Photo',
        color1: '#a0b890',
        color2: '#6a8a6a',
        comment: u.comment || '',
        mediaUrl: `/impressions/media/${encodeObjectPath(u.objectName)}`
      }));
    }
  } catch (_) {}
  ...
  return {
    ...base,
    ...data,
    items: [...galleryItems, ...uploadItems]
  };
}
