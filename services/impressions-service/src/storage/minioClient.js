...
async function putMediaObject({ objectName, buffer, contentType, comment }) {
  await ensureStorage();
  ...
  if (comment) {
    const json = Buffer.from(JSON.stringify({ comment: String(comment).substring(0, 500) }), 'utf8');
    await client.putObject(bucketName, `${objectName}.json`, json, json.length, {
...
async function listUploads() {
  ...
  const objects = await new Promise((resolve, reject) => {
    const items = [];
    const stream = client.listObjectsV2(bucketName, 'impressions/uploads/', true);
    stream.on('data', (obj) => items.push(obj));
    stream.on('end', () => resolve(items));
    stream.on('error', reject);
  });

  const imageObjects = objects.filter((o) => !o.name.endsWith('.json'));

  return Promise.all(imageObjects.map(async (obj) => {
    try {
      const stream = await client.getObject(bucketName, `${obj.name}.json`);
      const chunks = [];
      ...
      const { comment } = JSON.parse(Buffer.concat(chunks).toString('utf8'));
